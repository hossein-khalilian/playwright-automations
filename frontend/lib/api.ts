  ...
  (error) => {
    ...
    if (error.code === "ECONNABORTED" || error.message?.includes("timeout")) {
      error.response = {
        ...
        data: {
          detail:
            "Request timed out. The operation may still be processing. Please try again in a moment.",
        },
      };
    }
    ...
    if (
      errorDetail.includes("Failed to navigate") ||
      ...
      error.response = {
        ...
        data: {
          detail:
            "Browser is still navigating to the notebook. Please wait a moment and try again, or refresh the page.",
        },
      };
    }
